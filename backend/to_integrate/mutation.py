...
class SequenceAnalyzer:
    ...
    def _smith_waterman(self, seq1: str, seq2: str, match=2, mismatch=-1, gap=-1) -> Tuple[int, int, int]:
        ...
        for i in range(1, m + 1):
            ...
            diag = H[i-1, :-1] + np.where(
            np.frombuffer(seq2.encode(), dtype=np.uint8) == ord(seq1[i-1]),
            match, mismatch
        )
        # Can't fully vectorise due to gap dependency, but diagonal is the hot path
        ...
        max_score = int(H[max_idx])
        end_i = max_idx[0]

        # Traceback
        i, j = end_i, int(max_idx[1])
        while i > 0 and j > 0 and H[i, j] > 0:
            i -= 1
            j -= 1

        return max_score, i, end_i
